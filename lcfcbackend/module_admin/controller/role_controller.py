...
def get_system_role_dept_tree(request: Request,
                                    role_id: int,
...
def get_system_role_list(request: Request,
                               role_page_query: RolePageQueryModel = Depends(RolePageQueryModel.as_query),
                               query_db: Session = Depends(get_db),
                               data_scope_sql: str = Depends(GetDataScope('SysDept'))):
    role_page_query_result = RoleService.get_role_list_services(query_db, role_page_query, data_scope_sql,
                                                                      is_page=True)
    logger.info('获取成功')
...
def add_system_role(request: Request,
...
def edit_system_role(request: Request,
...
def edit_system_role_datascope(request: Request,
...
def delete_system_role(request: Request,
                             role_ids: str,
                             query_db: Session = Depends(get_db),
                             current_user: CurrentUserModel = Depends(LoginService.get_current_user),
                             data_scope_sql: str = Depends(GetDataScope('SysDept'))):
...
def query_detail_system_role(request: Request,
                                   role_id: int,
                                   query_db: Session = Depends(get_db),
                                   current_user: CurrentUserModel = Depends(LoginService.get_current_user),
                                   data_scope_sql: str = Depends(GetDataScope('SysDept'))):
    if not current_user.user.admin:
        RoleService.check_role_data_scope_services(query_db, str(role_id), data_scope_sql)
...
def export_system_role_list(request: Request,
                                  role_page_query: RolePageQueryModel = Form(),
                                  query_db: Session = Depends(get_db),
                                  data_scope_sql: str = Depends(GetDataScope('SysDept'))):
    # 获取全量数据
    role_query_result = RoleService.get_role_list_services(query_db, role_page_query, data_scope_sql,
                                                                 is_page=False)
...
@Log(title='角色管理', business_type=BusinessType.UPDATE)
def reset_system_role_status(request: Request,
...
@roleController.get('/authUser/allocatedList',
                    ...
                    dependencies=[Depends(CheckUserInterfaceAuth('system:role:list'))])
def get_system_allocated_user_list(
    request: Request,
...
@roleController.get('/authUser/unallocatedList',
                    response_model=PageResponseModel,
                    dependencies=[Depends(CheckUserInterfaceAuth('system:role:list'))])
def get_system_unallocated_user_list(request: Request,
                                           user_role: UserRolePageQueryModel = Depends(UserRolePageQueryModel.as_query),
                                           query_db: Session = Depends(get_db),
                                           data_scope_sql: str = Depends(GetDataScope('SysUser'))):
...
@Log(title='角色管理', business_type=BusinessType.GRANT)
def add_system_role_user(request: Request,
...
@Log(title='角色管理', business_type=BusinessType.GRANT)
def cancel_system_role_user(request: Request, cancel_user_role: CrudUserRoleModel,
                                  query_db: Session = Depends(get_db)):
...
@Log(title='角色管理', business_type=BusinessType.GRANT)
def batch_cancel_system_role_user(request: Request,
