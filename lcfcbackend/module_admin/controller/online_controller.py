...
def get_monitor_online_list(
    request: Request, online_page_query: OnlineQueryModel = Depends(OnlineQueryModel.as_query)
):
    # 获取全量数据
    online_query_result = OnlineService.get_online_list_services(request, online_page_query)
    logger.info('获取成功')
...
def delete_monitor_online(request: Request, token_ids: str, query_db: Session = Depends(get_db)):
