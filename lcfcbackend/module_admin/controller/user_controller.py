...
def get_system_dept_tree(request: Request, query_db: Session = Depends(get_db),
...
def get_system_user_list(request: Request,
    ...
    # 获取分页数据
    user_page_query_result = UserService.get_user_list_services(
...
def add_system_user(request: Request,
    ...
    if not current_user.user.admin:
        ...
        RoleService.check_role_data_scope_services(query_db, ','.join([str(item) for item in add_user.role_ids]),
                                                   role_data_scope_sql)
...
def edit_system_user(request: Request,
...
def delete_system_user(request: Request,
                             user_ids: str,
                             query_db: Session = Depends(get_db),
                             current_user: CurrentUserModel = Depends(LoginService.get_current_user),
                             data_scope_sql: str = Depends(GetDataScope('SysUser'))):
...
@Log(title='用户管理', business_type=BusinessType.UPDATE)
def reset_system_user_pwd(request: Request,
                                reset_user: EditUserModel,
                                query_db: Session = Depends(get_db),
                                current_user: CurrentUserModel = Depends(LoginService.get_current_user),
                                data_scope_sql: str = Depends(GetDataScope('SysUser'))):
    UserService.check_user_allowed_services(reset_user)
    if not current_user.user.admin:
        UserService.check_user_data_scope_services(query_db, reset_user.user_id, data_scope_sql)
    edit_user = EditUserModel(
...
@Log(title='用户管理', business_type=BusinessType.UPDATE)
def change_system_user_status(request: Request,
...
def query_detail_system_user_profile(request: Request,
                                           query_db: Session = Depends(get_db),
                                           current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
...
@userController.get('/',
                    response_model=UserDetailModel,
                    dependencies=[Depends(CheckUserInterfaceAuth('system:user:query'))])
def query_detail_system_user(request: Request,
                                   user_id: Optional[Union[int, Literal['']]] = '',
                                   query_db: Session = Depends(get_db),
                                   current_user: CurrentUserModel = Depends(LoginService.get_current_user),
                                   data_scope_sql: str = Depends(GetDataScope('SysUser'))):
    if user_id and not current_user.user.admin:
        UserService.check_user_data_scope_services(query_db, user_id, data_scope_sql)
...
def change_system_user_profile_avatar(request: Request,
                                            avatarfile: bytes = File(),
                                            query_db: Session = Depends(get_db),
                                            current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    if avatarfile:
        relative_path = f'avatar/{datetime.now().strftime("%Y/%m/%d")}'
...
@Log(title='个人信息', business_type=BusinessType.UPDATE)
def change_system_user_profile_info(request: Request,
...
@Log(title='个人信息', business_type=BusinessType.UPDATE)
def reset_system_user_password(request: Request,
...
def batch_import_system_user(request: Request,
                                   ...
                                   user_data_scope_sql: str = Depends(GetDataScope('SysUser')),
                                   dept_data_scope_sql: str = Depends(GetDataScope('SysDept'))):
    batch_import_result = UserService.batch_import_user_services(request, query_db, file, update_support,
                                                                       current_user, user_data_scope_sql,
                                                                       dept_data_scope_sql)
...
def export_system_user_template(request: Request, query_db: Session = Depends(get_db)):
...
def export_system_user_list(request: Request,
                                  user_page_query: UserPageQueryModel = Form(),
                                  query_db: Session = Depends(get_db),
                                  data_scope_sql: str = Depends(GetDataScope('SysUser'))):
    # 获取全量数据
    user_query_result = UserService.get_user_list_services(query_db, user_page_query, data_scope_sql,
                                                                 is_page=False)
...
@userController.get('/authRole/{user_id}',
                    ...
                    dependencies=[Depends(CheckUserInterfaceAuth('system:user:query'))])
def get_system_allocated_role_list(request: Request, user_id: int, query_db: Session = Depends(get_db)):
    user_role_query = UserRoleQueryModel(userId=user_id)
    user_role_allocated_query_result = UserService.get_user_role_allocated_list_services(
...
def update_system_role_user(request: Request,
                                  ...
                                  role_data_scope_sql: str = Depends(GetDataScope('SysDept'))):
    if not current_user.user.admin:
        UserService.check_user_data_scope_services(query_db, user_id, user_data_scope_sql)
        RoleService.check_role_data_scope_services(query_db, role_ids, role_data_scope_sql)
    add_user_role_result = UserService.add_user_role_services(
