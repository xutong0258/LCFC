...
@Log(title='定时任务', business_type=BusinessType.UPDATE)
def execute_system_job(request: Request, execute_job: JobModel, query_db: Session = Depends(get_db)):
...
def delete_system_job(request: Request, job_ids: str, query_db: Session = Depends(get_db)):
...
def query_detail_system_job(request: Request, job_id: int, query_db: Session = Depends(get_db)):
...
def export_system_job_list(
    ...
    job_export_result = JobService.export_job_list_services(request, job_query_result)
    logger.info('导出成功')
...
def clear_system_job_log(request: Request, query_db: Session = Depends(get_db)):
...
def delete_system_job_log(request: Request, job_log_ids: str, query_db: Session = Depends(get_db)):
...
def export_system_job_log_list(
    request: Request,
    ...
    job_log_export_result = JobLogService.export_job_log_list_services(request, job_log_query_result)
    logger.info('导出成功')
