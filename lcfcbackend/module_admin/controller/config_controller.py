...
def edit_system_config(
    ...
    edit_config_result = ConfigService.edit_config_services(request, query_db, edit_config)
...
@Log(title='参数管理', business_type=BusinessType.UPDATE)
def refresh_system_config(request: Request, query_db: Session = Depends(get_db)):
...
def delete_system_config(request: Request, config_ids: str, query_db: Session = Depends(get_db)):
...
def query_detail_system_config(request: Request, config_id: int, query_db: Session = Depends(get_db)):
...
def query_system_config(request: Request, config_key: str):
    # 获取全量数据
    config_query_result = ConfigService.query_config_list_from_cache_services(request.app.state.redis, config_key)
    logger.info('获取成功')
