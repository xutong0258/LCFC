...
class RoleDao:
    ...
    def get_role_list(
        ...
        role_list = PageUtil.paginate(db, query, query_object.page_num, query_object.page_size, is_page)
    ...
    @classmethod
    def get_role_menu_dao(cls, db: Session, role: RoleModel):
        """
        根据角色id获取角色菜单关联列表信息
    ...
    @classmethod
    def get_role_dept_dao(cls, db: Session, role: RoleModel):
        """
        根据角色id获取角色部门关联列表信息
    ...
    @classmethod
    def count_user_role_dao(cls, db: Session, role_id: int):
        """
        根据角色id查询角色关联用户数量
