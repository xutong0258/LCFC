...
class ConfigDao:
    ...
    def get_config_list(cls, db: Session, query_object: ConfigPageQueryModel, is_page: bool = False):
        ...
        query = (select(SysConfig)
            ...
            if query_object.begin_time and query_object.end_time else True).order_by(SysConfig.config_id).distinct())
        config_list = PageUtil.paginate(db, query, query_object.page_num, query_object.page_size, is_page)
        return config_list
