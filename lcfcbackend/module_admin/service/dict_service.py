...
class DictTypeService:
    ...
    @classmethod
    def check_dict_type_unique_services(cls, query_db:  Session, page_object: DictTypeModel):
        """
        校验字典类型称是否唯一service
    ...
    @classmethod
    def add_dict_type_services(cls, request: Request, query_db:  Session, page_object: DictTypeModel):
        """
        新增字典类型信息service
        ...
        if not cls.check_dict_type_unique_services(query_db, page_object):
            ...
            try:
                ...
                query_db.commit()
                request.app.state.redis.set(f'{RedisInitKeyConfig.SYS_DICT.key}:{page_object.dict_type}', '')
    ...
    @classmethod
    def edit_dict_type_services(cls, request: Request, query_db:  Session, page_object: DictTypeModel):
        """
        编辑字典类型信息service
        ...
        if page_object.dict_ids:
            ...
            try:
                ...
                if delete_dict_type_list:
                    request.app.state.redis.delete(*delete_dict_type_list)
    ...
    @classmethod
    def refresh_sys_dict_services(cls, request: Request, query_db:  Session):
        """
        刷新字典缓存信息service
        ...
        :return: 刷新字典缓存校验结果
        """
        DictDataService.init_cache_sys_dict_services(query_db, request.app.state.redis)
...
class DictDataService:
    ...
    @classmethod
    def init_cache_sys_dict_services(cls, query_db: Session, redis):
        """
        ...
        """
        # 获取以sys_dict:开头的键列表
        keys = redis.keys(f'{RedisInitKeyConfig.SYS_DICT.key}:*')
        # 删除匹配的键
        if keys:
            redis.delete(*keys)
        ...
        for dict_type_obj in [item for item in dict_type_all if item.status == '0']:
            ...
            dict_data = [CamelCaseUtil.transform_result(row) for row in dict_data_list if row]
            redis.set(f'{RedisInitKeyConfig.SYS_DICT.key}:{dict_type}',
                      json.dumps(dict_data, ensure_ascii=False, default=str))

    @classmethod
    def query_dict_data_list_from_cache_services(cls, redis, dict_type: str):
        """
        ...
        """
        result = []
        dict_data_list_result = redis.get(f'{RedisInitKeyConfig.SYS_DICT.key}:{dict_type}')
    ...
    @classmethod
    def check_dict_data_unique_services(cls, query_db:  Session, page_object: DictDataModel):
        """
        校验字典数据是否唯一service
    ...
    @classmethod
    def add_dict_data_services(cls, request: Request, query_db:  Session, page_object: DictDataModel):
        """
        新增字典数据信息service
        ...
        if not cls.check_dict_data_unique_services(query_db, page_object):
            ...
            try:
                ...
                query_db.commit()
                dict_data_list = cls.query_dict_data_list_services(query_db, page_object.dict_type)
                request.app.state.redis.set(
                    f'{RedisInitKeyConfig.SYS_DICT.key}:{page_object.dict_type}',
    ...
    @classmethod
    def edit_dict_data_services(cls, request: Request, query_db:  Session, page_object: DictDataModel):
        """
        编辑字典数据信息service
        ...
        if page_object.dict_codes:
            ...
            try:
                ...
                for dict_type in list(set(delete_dict_type_list)):
                    dict_data_list = cls.query_dict_data_list_services(query_db, dict_type)
                    request.app.state.redis.set(
