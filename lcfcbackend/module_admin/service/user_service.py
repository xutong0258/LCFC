...
from exceptions.exception import ServiceException, NotFoundException
...
class UserService:
    ...
    @classmethod
    def check_user_allowed_services(cls, check_user: UserModel):
        """
        校验用户是否允许操作service
    ...
    @classmethod
    def check_user_data_scope_services(cls, query_db:  Session, user_id: int, data_scope_sql: str):
        """
        校验用户数据权限service
        ...
        :return: 校验结果
        """
        users = UserDao.get_user_list(query_db, UserPageQueryModel(userId=user_id), data_scope_sql, is_page=False)
        if users:
            return CrudResponseModel(is_success=True, message='校验通过')
        else:
    ...
    @classmethod
    def check_user_name_unique_services(cls, query_db:  Session, page_object: UserModel):
        """
        校验用户名是否唯一service
    ...
    @classmethod
    def check_phonenumber_unique_services(cls, query_db:  Session, page_object: UserModel):
        """
        校验用户手机号是否唯一service
    ...
    @classmethod
    def check_email_unique_services(cls, query_db:  Session, page_object: UserModel):
        """
        校验用户邮箱是否唯一service
    ...
    @classmethod
    def batch_import_user_services(
        cls,
        request: Request,
        query_db:  Session,
        file: UploadFile,
    ...
    def get_user_role_allocated_list_services(cls, query_db:  Session, page_object: UserRoleQueryModel):
        ...
        query_role_list = [
            SelectedRoleModel(**row) for row in RoleService.get_role_select_option_services(query_db)
        ]
