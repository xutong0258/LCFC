...
class OnlineService:
    ...
    @classmethod
    def get_online_list_services(cls, request: Request, query_object: OnlineQueryModel):
        """
        获取在线用户表信息service
        ...
        :return: 在线用户列表信息
        """
        access_token_keys = request.app.state.redis.keys(f'{RedisInitKeyConfig.ACCESS_TOKEN.key}*')
        if not access_token_keys:
            access_token_keys = []
        access_token_values_list = [request.app.state.redis.get(key) for key in access_token_keys]
        online_info_list = []
    ...
    @classmethod
    def delete_online_services(cls, request: Request, page_object: DeleteOnlineModel):
        """
        强退在线用户信息service
        ...
        if page_object.token_ids:
            ...
            for token_id in token_id_list:
                request.app.state.redis.delete(f'{RedisInitKeyConfig.ACCESS_TOKEN.key}:{token_id}')
