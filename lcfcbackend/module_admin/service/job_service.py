...
class JobService:
    ...
    def get_job_list_services(
        cls, query_db:  Session, query_object: JobPageQueryModel, is_page: bool = False
    ):
        """
        获取定时任务列表信息service
    ...
    @classmethod
    def check_job_unique_services(cls, query_db:  Session, page_object: JobModel):
        """
        校验定时任务是否存在service
    ...
    def add_job_services(cls, query_db:  Session, page_object: JobModel):
        ...
        elif not cls.check_job_unique_services(query_db, page_object):
            raise ServiceException(message=f'新增定时任务{page_object.job_name}失败，定时任务已存在')
        else:
            try:
                add_job = JobDao.add_job_dao(query_db, page_object)
                job_info = cls.job_detail_services(query_db, add_job.job_id)
    ...
    @staticmethod
    def export_job_list_services(request: Request, job_list: List):
        """
        导出定时任务信息service
        ...
        data = job_list
        job_group_list = DictDataService.query_dict_data_list_from_cache_services(
        ...
        job_executor_list = DictDataService.query_dict_data_list_from_cache_services(
