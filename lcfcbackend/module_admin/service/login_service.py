...
class LoginService:
    ...
    @classmethod
    def authenticate_user(cls, request: Request, query_db:  Session, login_user: UserLogin):
        """
        根据用户名密码校验用户登录
        ...
        :return: 校验结果
        """
        cls.__check_login_ip(request)
        account_lock = request.app.state.redis.get(
        ...
        if not login_user.captcha_enabled or (
            ...
            cls.__check_login_captcha(request, login_user)
        ...
        if not PwdUtil.verify_password(login_user.password, user[0].password):
            cache_password_error_count = request.app.state.redis.get(
        ...
        request.app.state.redis.delete(f'{RedisInitKeyConfig.PASSWORD_ERROR_COUNT.key}:{login_user.user_name}')
        return user

    @classmethod
    def __check_login_ip(cls, request: Request):
        """
        校验用户登录ip是否在黑名单内
        ...
        :return: 校验结果
        """
        black_ip_value = request.app.state.redis.get(f'{RedisInitKeyConfig.SYS_CONFIG.key}:sys.login.blackIPList')
    ...
    @classmethod
    def __check_login_captcha(cls, request: Request, login_user: UserLogin):
        """
        校验用户登录验证码
        ...
        :return: 校验结果
        """
        captcha_value = request.app.state.redis.get(f'{RedisInitKeyConfig.CAPTCHA_CODES.key}:{login_user.uuid}')
    ...
    def get_current_user(
        ...
        if AppConfig.app_same_time_login:
            redis_token = request.app.state.redis.get(f'{RedisInitKeyConfig.ACCESS_TOKEN.key}:{session_id}')
        else:
            # 此方法可实现同一账号同一时间只能登录一次
            redis_token = request.app.state.redis.get(
    ...
    @classmethod
    def get_current_user_routers(cls, user_id: int, query_db:  Session):
        """
        根据用户id获取当前用户路由信息
    ...
    @classmethod
    def register_user_services(cls, request: Request, query_db:  Session, user_register: UserRegister):
        """
        用户注册services
        ...
        register_enabled = (
            True
            if request.app.state.redis.get(f'{RedisInitKeyConfig.SYS_CONFIG.key}:sys.account.registerUser')
            == 'true'
            else False
        )
        captcha_enabled = (
            True
            if request.app.state.redis.get(f'{RedisInitKeyConfig.SYS_CONFIG.key}:sys.account.captchaEnabled')
            == 'true'
            else False
        )
        if user_register.password == user_register.confirm_password:
            if register_enabled:
                if captcha_enabled:
                    captcha_value = request.app.state.redis.get(
    ...
    @classmethod
    def get_sms_code_services(cls, request: Request, query_db:  Session, user: ResetUserModel):
        """
        获取短信验证码service
        ...
        :return: 短信验证码对象
        """
        redis_sms_result = request.app.state.redis.get(f'{RedisInitKeyConfig.SMS_CODE.key}:{user.session_id}')
        if redis_sms_result:
    ...
    @classmethod
    def forget_user_services(cls, request: Request, query_db:  Session, forget_user: ResetUserModel):
        """
        用户忘记密码services
        ...
        :return: 重置结果
        """
        redis_sms_result = request.app.state.redis.get(
        ...
        if forget_user.sms_code == redis_sms_result:
            ...
            request.app.state.redis.delete(f'{RedisInitKeyConfig.SMS_CODE.key}:{forget_user.session_id}')
            result = dict(is_success=False, message='短信验证码不正确')

        return CrudResponseModel(**result)

    @classmethod
    def logout_services(cls, request: Request, session_id: str):
        """
        退出登录services
        ...
        :return: 退出登录结果
        """
        request.app.state.redis.delete(f'{RedisInitKeyConfig.ACCESS_TOKEN.key}:{session_id}')
        # request.app.state.redis.delete(f'{current_user.user.user_id}_access_token')
        # request.app.state.redis.delete(f'{current_user.user.user_id}_session_id')
