...
class ConfigService:
    ...
    @classmethod
    def init_cache_sys_config_services(cls, query_db: Session, redis):
        """
        ...
        keys = redis.keys(f'{RedisInitKeyConfig.SYS_CONFIG.key}:*')
        # 删除匹配的键
        if keys:
            redis.delete(*keys)
        config_all = ConfigDao.get_config_list(query_db, ConfigPageQueryModel(**dict()), is_page=False)
        for config_obj in config_all:
            redis.set(f"{RedisInitKeyConfig.SYS_CONFIG.key}:{config_obj.get('configKey')}",
                      config_obj.get('configValue'))

    @classmethod
    def query_config_list_from_cache_services(cls, redis, config_key: str):
        """
        从缓存获取参数键名对应值service
        ...
        :return: 参数键名对应值
        """
        result = redis.get(f'{RedisInitKeyConfig.SYS_CONFIG.key}:{config_key}')

        return result

    @classmethod
    def check_config_key_unique_services(cls, query_db:  Session, page_object: ConfigModel):
        """
        校验参数键名是否唯一service
    ...
    @classmethod
    def add_config_services(cls, request: Request, query_db:  Session, page_object: ConfigModel):
        """
        新增参数配置信息service
    ...
    @classmethod
    def edit_config_services(cls, request: Request, query_db:  Session, page_object: ConfigModel):
        """
        编辑参数配置信息service
        ...
        config_info = cls.config_detail_services(query_db, page_object.config_id)
    ...
    @classmethod
    def delete_config_services(cls, request: Request, query_db:  Session, page_object: DeleteConfigModel):
        """
        删除参数配置信息service
        ...
        if page_object.config_ids:
            ...
            try:
                ...
                if delete_config_key_list:
                    request.app.state.redis.delete(*delete_config_key_list)
    ...
    @classmethod
    def refresh_sys_config_services(cls, request: Request, query_db:  Session):
        """
        刷新字典缓存信息service
        ...
        :return: 刷新字典缓存校验结果
        """
        cls.init_cache_sys_config_services(query_db, request.app.state.redis)
