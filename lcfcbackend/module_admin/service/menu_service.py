...
class MenuService:
    ...
    def get_menu_tree_services(cls, query_db:  Session, current_user: Optional[CurrentUserModel] = None):
        ...
        :return: 菜单树信息对象
        """
        menu_list_result = MenuDao.get_menu_list_for_tree(
    ...
    @classmethod
    def get_role_menu_tree_services(
        ...
        menu_tree_result = cls.list_to_tree(menu_list_result)
        role = RoleDao.get_role_detail_by_id(query_db, role_id)
        role_menu_list = RoleDao.get_role_menu_dao(query_db, role)
    ...
    def get_menu_list_services(
        ...
        :return: 菜单列表信息对象
        """
        menu_list_result = MenuDao.get_menu_list(
    ...
    @classmethod
    def check_menu_name_unique_services(cls, query_db:  Session, page_object: MenuModel):
        """
        校验菜单名称是否唯一service
        ...
        menu = MenuDao.get_menu_detail_by_info(query_db, MenuModel(menuName=page_object.menu_name))
    ...
    def add_menu_services(cls, query_db:  Session, page_object: MenuModel):
        ...
        :return: 新增菜单校验结果
        """
        if not cls.check_menu_name_unique_services(query_db, page_object):
    ...
    def edit_menu_services(cls, query_db:  Session, page_object: MenuModel):
        ...
        menu_info = cls.menu_detail_services(query_db, page_object.menu_id)
        ...
        if page_object.menu_ids:
            ...
            try:
                for menu_id in menu_id_list:
                    if (MenuDao.has_child_by_menu_id_dao(query_db, int(menu_id))) > 0:
                        raise ServiceWarning(message='存在子菜单,不允许删除')
                    elif (MenuDao.check_menu_exist_role_dao(query_db, int(menu_id))) > 0:
